...
dat_csv_temp <- read.csv(file = "Scopus/NPS-LegalHighs/ScopusNPS.csv")

dat_csv_temp2 <- dat_csv_temp

##### Dataset corrections #####
# Replace "United States" with USA & "United Kingdom" with UK
dat_csv_temp2$Affiliations <- gsub("United States", "USA", dat_csv_temp2$Affiliations)
dat_csv_temp2$Affiliations <- gsub("United Kingdom", "UK", dat_csv_temp2$Affiliations)
dat_csv_temp2$Affiliations <- gsub("\\.", "", dat_csv_temp2$Affiliations)
dat_csv_temp2$Affiliations <- gsub("USA\\, United States", "USA", dat_csv_temp2$Affiliations)
dat_csv_temp2$Affiliations <- gsub("Russian Federation", "Russia", dat_csv_temp2$Affiliations)
dat_csv_temp2$Affiliations <- gsub("Cairo Egypt", "Cairo\\, Egypt", dat_csv_temp2$Affiliations)
dat_csv_temp2$Affiliations <- gsub("Hong Kong", "Hong Kong\\, China", dat_csv_temp2$Affiliations)

# remove diacritics from affiliations
dat_csv_temp2$Affiliations <- removeDiacritics(dat_csv_temp2$Affiliations)
...
dat_csv_temp2$Affiliations <-  mgsub(dat_csv_temp2$Affiliations, CityOriginalName, CityEditedName)

##replace country name containing space with equivalent
dat_csv_temp2$Affiliations <-  mgsub(dat_csv_temp2$Affiliations, CountryOriginalName, CountryEditedName)

# keeping Title as reference for grouping (i.e. relationship between cities), select Affiliations 
AffiliationsNarrow <- dat_csv_temp2 %>%
...
# remove rows with NA in lattitude and longitude
