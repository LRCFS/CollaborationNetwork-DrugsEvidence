...
cit.path.SCOPUS.DRUGS.SEIZURE <- "Scopus/Drugs-Forensic-Seizure/"

filenames <- list.files(cit.path.SCOPUS.DRUGS.SEIZURE, pattern=extension, full.names=TRUE)

dat_csv <- rbindlist(lapply(filenames,fread, encoding='UTF-8'))
...
#save figure
Var1 <- paste0("Fig_2_Scopus_Seizure",KeywordEntries)
 show(p)
